...
if($widget->properties['showBox']):
 $this->beginWidget('UIPanel', array('label'=>$widget->label, 
		'icond'=>'ico-pencil','labelTags'=>$widget->labelTags));
endif;
?>
...
<?php foreach($widget->groups as $groupIndex => $group){?>
    	...
    	<?php if($group['label']):?>
        	<legend><?php echo $group['label']?>
        <?php if($group['id']!='DEFAULT'):?>
        	<a style="text-decoration:underline;cursor:hand;" data-toggle="collapse"
 href="#fieldset_<?php echo $widget->getId().'_'.$group['id']?>">
 <small class="label pull-right">展开/收缩选项</small></a>
        
        <?php endif?>
        </legend>
        <?php endif?>
        <div <?php if(!$group['isshow']){?>style="height:0px;"<?php }?> id="fieldset_<?php echo $widget->GetId().'_'.$group['id']?>">
	<?php
	if(is_array($widget->elements[$group['id']])):
		foreach($widget->elements[$group['id']] as $elIndex => $field):?>
			...
			<div class="control-group"><?php echo $field->GetBody()?></div>
			<?php else:?>
